...
class CrewMemberStore {
  ...
  async getAll() {
    try {
      const response = await fetch(`${SERVER}/movies/${this.movieId}/crew-members`)
  ...
  async addOne(crewMember) {
    try {
      await fetch(`${SERVER}/movies/${this.movieId}/crew-members`, {
        method: 'post',
        headers: {
  ...
  async deleteOne(id) {
    try {
      await fetch(`${SERVER}/movies/${this.movieId}/crew-members/${id}`, {
        method: 'delete'
      })
      this.getAll()
    } catch (err) {
